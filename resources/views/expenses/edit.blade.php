                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('expenses.update', $expense) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <div class="flex items-start mt-1">
                                    <div class="flex items-center h-5">
                                        <input id="is_recurring" name="is_recurring" type="checkbox" value="1" {{ old('is_recurring', $expense->is_recurring) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800">
                                    </div>
                            ...
                            <!-- Recurring Frequency -->
                            <div class="md:col-span-2 {{ $expense->is_recurring ? '' : 'hidden' }}" id="recurring_frequency_container">
                                ...
                                <select id="recurring_frequency" name="recurring_frequency" class="block mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 shadow-sm">
                                    <option value="daily" {{ old('recurring_frequency', $expense->recurring_frequency) == 'daily' ? 'selected' : '' }}>{{ __('Daily') }}</option>
                                    <option value="weekly" {{ old('recurring_frequency', $expense->recurring_frequency) == 'weekly' ? 'selected' : '' }}>{{ __('Weekly') }}</option>
                                    <option value="biweekly" {{ old('recurring_frequency', $expense->recurring_frequency) == 'biweekly' ? 'selected' : '' }}>{{ __('Biweekly') }}</option>
                                    <option value="monthly" {{ old('recurring_frequency', $expense->recurring_frequency) == 'monthly' ? 'selected' : '' }}>{{ __('Monthly') }}</option>
                                    <option value="quarterly" {{ old('recurring_frequency', $expense->recurring_frequency) == 'quarterly' ? 'selected' : '' }}>{{ __('Quarterly') }}</option>
                                    <option value="yearly" {{ old('recurring_frequency', $expense->recurring_frequency) == 'yearly' ? 'selected' : '' }}>{{ __('Yearly') }}</option>
                                </select>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="notes" name="notes" rows="3" class="block mt-1 w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 shadow-sm">{{ old('notes', $expense->notes) }}</textarea>
